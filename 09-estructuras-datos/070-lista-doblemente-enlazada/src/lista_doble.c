...
#if !defined(CRITERION_NO_MAIN) && !defined(DEBUG_TEST)
int main(void) {
    printf("=== EJERCICIO 070: LISTA DOBLEMENTE ENLAZADA ===\n");
    ...
    // Insertar elementos como en el enunciado
    lista_insertar_final(lista, 10);
    lista_insertar_final(lista, 20);
    lista_insertar_final(lista, 30);
    ...
    // Validar integridad
    printf("Integridad de enlaces: %s\n", 
           lista_validar_integridad(lista) ? "Correcta" : "Incorrecta");
    
    printf("========================================\n");
    
    // Demostraciones adicionales
    lista_limpiar(lista);  // Limpiar para demos
    
    demo_operaciones_basicas(lista);
    lista_limpiar(lista);
    
    demo_recorridos_bidireccionales(lista);
    lista_limpiar(lista);
    
    demo_iteradores(lista);
    lista_limpiar(lista);
    
    demo_operaciones_avanzadas(lista);
    
    // Mostrar estadísticas finales
    lista_mostrar_estadisticas(lista);
    lista_mostrar_estructura(lista);
